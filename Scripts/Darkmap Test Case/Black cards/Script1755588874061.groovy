...
String readIsSeenSmart(int timeoutMs = 4000) {
    ...
    // son çare: klasik okuma (kısa timeout)
...
void closeAllSeenOnPage(){
    ...
    while(true){
        ...
        for(WebElement sw : onList){
            try { ((JavascriptExecutor)DriverFactory.getWebDriver()).executeScript("arguments[0].scrollIntoView({block:'center'});", sw) } catch (Exception ignore){}
...
void runFlowOnCurrentPage(){
    ...
    WebUI.delay(0.4); waitGoneOverlay()
