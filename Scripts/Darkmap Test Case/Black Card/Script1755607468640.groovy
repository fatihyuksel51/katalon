...
void waitGoneOverlay(){ waitOverlayGone(10) } // senin helper’ın
...
// ---------- Sabitler (Field olarak!) ----------
@Field final String PAGE_URL            = "https://platform.catchprobe.org/darkmap/blackcard"
@Field final String ROW_XP              = "//div[contains(@class,'group') and contains(@class,'grid-cols-24') and contains(@class,'bg-card')]"
...
void closeAllSeenOnPage(){
    ...
    while(true){
        ...
        for(WebElement sw : onList){
            safeScrollTo(xp(LIST_SWITCH_ON_XP), 5) // kabaca ortalara scroll
            try { sw.click() } catch(e){ jsClick(sw) }
            WebUI.delay(0.3)
        }
        if(++guard > 10) break // sonsuz döngü koruması
    }
}

String readFormIsSeen(){ return $(FORM_IS_SEEN_VAL, 10).getText()?.trim() }
...
WebElement expectSingleRow(){
    WebUI.waitForElementVisible(xp(ROW_XP), 10)
    List<WebElement> rows = $$(ROW_XP, 5)
    assert rows.size()==1 : "Tek satır bekleniyordu, bulunan: ${rows.size()}"
    return rows.get(0)
}
...
void runFlowOnCurrentPage(){
    ...
    String seenVal0 = readFormIsSeen()
    ...
    waitToast(TOAST_SEEN_OK, 8); WebUI.delay(0.5)

    // 4) Doğrula (Yes)
    WebElement eye = row.findElement(By.xpath(ROW_EYE_XP))
    try { eye.click() } catch(e){ jsClick(eye) }
    waitGoneOverlay()
    String seenVal1 = readFormIsSeen()
    assert seenVal1.equalsIgnoreCase("Yes") : "Detay formunda Is Seen 'Yes' bekleniyordu, bulunan: ${seenVal1}"
    pressEsc(); WebUI.delay(0.6)

    // 5) Filter: Yes
    clickTO(xp(SEEN_SELECT_XP)); WebUI.delay(1)
    clickTO(xp(OPT_YES_XP)); WebUI.delay(0.4)
    clickTO(xp(SEARCH_BTN_XP)); waitGoneOverlay()
    ...
    if(WebUI.waitForElementPresent(xp(EXPORT_CSV_BTN_XP), 5, FailureHandling.OPTIONAL)){
        ...
        waitToast(TOAST_EXPORT_OK, 8)
    } else { KeywordUtil.logInfo("Export CSV butonu bulunamadı; metin farklı olabilir.") }
