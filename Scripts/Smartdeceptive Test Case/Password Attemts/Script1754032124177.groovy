...
void passwordAttemtsTest() {
    // 🔹 İlk satır verilerini al
	TestObject attackNameObj = makeXpathObj("(.//td[contains(@class, 'ant-table-cell')])[9]")
	TestObject ipAddressObj  = makeXpathObj("(.//td[contains(@class, 'ant-table-cell')])[4]")
	TestObject timeStampObj  = makeXpathObj("(.//td[contains(@class, 'ant-table-cell')])[8]")
   ...
   String sortedTime = WebUI.getText(makeXpathObj("(.//td[contains(@class, 'ant-table-cell')])[8]"))
...
TestObject ipCellObj = makeXpathObj("(.//td[contains(@class, 'ant-table-cell')])[4]")
TestObject UsernameKeyword = makeXpathObj("(.//td[contains(@class, 'ant-table-cell')])[1]")
...
if (page3AttackName == null || page3AttackName.trim().toLowerCase() == "n/a") {
...
void verifyPagination(int totalCount) {
    ...
    WebUI.waitForElementVisible(pageLinkObj, 5)
